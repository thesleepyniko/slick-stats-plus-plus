...
PORT             = 6768 # :sixseven:
APP_SUPPORT_PATH = Path.home() / "Library" / "Application Support" / "Slick Stats++"
APP_CONFIG_PATH  = APP_SUPPORT_PATH / "config.json"
    ...
    def do_GET(self):
        ...
        if state != STATE:
            ...
            if not data.get("enterprise", {}).get("id") or data.get("enterprise", {}).get("id") != "E09V59WQY1E": 
                ...
                APP_CONFIG_PATH.write_text(json.dumps({"access_token": access_token}, indent=2))
                ...
                self.send_header("Content-Type", "text/html; charset=utf-8")
                self.end_headers()
                html = "<html><body><h1>200</h1><h2>Successfully authenticated!</h2></body></html>"
                self.wfile.write(html.encode("utf-8"))
...
with http.server.HTTPServer(('localhost', PORT), OAuthCallbackHandler) as httpd:
    print("DEBUG: Starting server to handle callback.")
    while not getattr(httpd, '_shutdown', False):
        httpd.handle_request()
    
    if getattr(httpd, '_error', True):
        print("DEBUG: Error handled, shutting down...")
        sys.exit(1)
    else:
        print("DEBUG: Successful auth, shutting down...")
        sys.exit(0)
